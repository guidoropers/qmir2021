...
Let's take a look at our data `df_learnr12`. We want you to create a dummy variable for democracy. The dummy should take the value "1" if the "democ"-variable takes a value greater than 6. 


```{r exercise1, exercise = T}
# Create a dummy variable for countries that score higher that 6 on the "democ"-variable.

```

### Modelling

Replicate the pooled model from our lab session but include an interaction for the duration of World Bank membership and the newly created dummy. What is the coefficient of the interaction?
...
Run the same model, but include a fixed-effect for the unit. Our unit is the country (`country_name`) What is the coefficient for the dummy now?
...
In a final step, we want to calculate predicted values. Remember from the last sessions that predicted values can help us to communicate the results of our analysis. In the following section, the model has already been specified. Please calculate predicted values for membership in the **World Bank**. Specify scenarios for Uruguay and Japan. In which of the two countries is the predicted value for the maximum duration of World Bank membership higher?

Include a unit fixed effect using the `country_name` variable.


```{r exercise4, exercise = TRUE}

model1 <- lm(n_RAI ~ length_EU + length_world + democ + gdp_capita + population_size + nationalization + country_name, data = df_learnr12)
